...
async fn get_all_blocks(blockchain: Arc<RwLock<Blockchain>>) -> String {
    let chain = blockchain.read().await;
...
async fn handle_balance_request(blockchain: Arc<RwLock<Blockchain>>, path: &str) -> String {
...
async fn get_chain_info(blockchain: Arc<RwLock<Blockchain>>) -> String {
    let chain = blockchain.read().await;
...
async fn get_blocks_from(blockchain: Arc<RwLock<Blockchain>>, from_height: u64) -> String {
    let chain = blockchain.read().await;
    let blocks: Vec<_> = chain.chain.iter().skip(from_height as usize).collect();
...
async fn handle_new_block(blockchain: Arc<RwLock<Blockchain>>, body: &str) -> String {
