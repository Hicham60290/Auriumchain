...
impl SecureWallet {
    pub fn generate(name: String, password: &str, address_type: &str) -> Result<Self, String> {
        ...
        let (encrypted_private_key, encrypted_seed, nonce, salt) = Self::encrypt_sensitive_data(
            &private_key_bytes,
        ...
        let integrity_hash =
            Self::calculate_integrity_hash(&encrypted_private_key, &encrypted_seed, &nonce, &salt);
    ...
    pub fn decrypt_private_key(&self, password: &str) -> Result<Vec<u8>, String> {
        ...
        let salt = PasswordHash::new(&self.salt).map_err(|e| format!("Invalid salt: {}", e))?;
    ...
    pub fn decrypt_seed(&self, password: &str) -> Result<String, String> {
        ...
        String::from_utf8(decrypted).map_err(|_| "Invalid seed data".to_string())
    }
    ...
    pub fn save(&self, directory: &str) -> Result<String, String> {
        ...
        fs::write(&filename, json).map_err(|e| format!("Failed to write wallet: {}", e))?;
    ...
    pub fn load(filename: &str) -> Result<Self, String> {
        let content =
            fs::read_to_string(filename).map_err(|e| format!("Failed to read wallet: {}", e))?;

        let wallet: SecureWallet =
            serde_json::from_str(&content).map_err(|e| format!("Failed to parse wallet: {}", e))?;
